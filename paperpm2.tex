...
\author{
\IEEEauthorblockN{R. P. Huallpa Yapo\IEEEauthorrefmark{1}, L. F. Temoche Dedios\IEEEauthorrefmark{1}, A. K. Nieves Acosta\IEEEauthorrefmark{1}, C. H. Inga Espinoza\IEEEauthorrefmark{1}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}Faculty of Mechatronics Engineering, Universidad Peruana de Ciencias Aplicadas, Lima, Peru\\
Email: \{u20201c125, u20181c826, pcmcanie, pcelcain\}@upc.edu.pe}
}
...
% (4) RESULTADOS CLAVE
El modelo propuesto predijo la tasa de rotura y la distribución de tamaño de partícula (PSD) del producto con un error medio absoluto inferior al 8\%, y reprodujo la tendencia experimental al aumentar la velocidad del rodillo y disminuir el espacio rodillo--criba.
...
La pulidora utilizada en este estudio es un equipo horizontal abrasivo de escala de laboratorio desarrollado en la Universidad Peruana de Ciencias Aplicadas.
Los experimentos de validación se realizaron sobre la pulidora de laboratorio en $N=9$ combinaciones de parámetros operativos (velocidad de rodillo $\times$ espacio $\times$ velocidad angular del blanqueador), cubriendo el rango operativo del equipo. Para cada condición se procesó un lote de 500\,g de arroz cargo y el producto se tamizó para obtener la PSD experimental.
...
Las simulaciones DEM-PRM con el modelo de Tavares permitieron predecir la tasa de rotura de grano y la distribución de tamaño de partícula (PSD) bajo diferentes combinaciones de velocidad de rodillo, espacio rodillo--criba y velocidad angular del blanqueador. Los resultados preliminares muestran que un aumento en la velocidad del rodillo incrementa la tasa de rotura, mientras que una reducción en el espacio rodillo--criba también eleva la proporción de granos partidos. La Figura~\ref{fig:simulacion} ilustra la evolución de la PSD simulada para tres condiciones operativas representativas.

\begin{figure}[h]
	\centering
	%\includegraphics[width=0.45\textwidth]{simulacion_psd.png}
	\caption{Distribución de tamaño de partícula (PSD) simulada bajo diferentes condiciones operativas.}
	\label{fig:simulacion}
\end{figure}

\subsection{Resultados Experimentales}
Los experimentos de validación realizados en la pulidora instrumentada muestran una tendencia similar a la predicha por la simulación. En la Tabla~\ref{tab:resultados}, se resumen los valores preliminares de tasa de rotura y MAPE para tres combinaciones de parámetros operativos. Se observa que el error porcentual absoluto medio entre los resultados simulados y experimentales es inferior al 10\% en todos los casos evaluados.

\begin{table}[h]
	\centering
	\caption{Resultados preliminares de tasa de rotura y MAPE para tres condiciones operativas.}
	\label{tab:resultados}
	\renewcommand{\arraystretch}{1.1}
	\begin{tabular}{C{1.2cm} C{1.2cm} C{1.2cm} C{1.2cm} C{1.2cm}}
		\toprule
		$\omega_b$ (rpm) & $g$ (mm) & Tasa Sim. (\%) & Tasa Exp. (\%) & MAPE (\%) \\
		\midrule
		1200 & 2.0 & 18.5 & 20.1 & 7.9 \\
		1500 & 1.5 & 25.3 & 27.0 & 6.3 \\
		1800 & 1.0 & 32.7 & 35.2 & 7.1 \\
		\bottomrule
	\end{tabular}
\end{table}

Estos resultados preliminares validan la capacidad del modelo DEM-PRM calibrado para predecir la rotura de grano bajo condiciones operativas variables y sientan las bases para una optimización futura del proceso.
% ────────────────────────────── ───────────────
% ─────────────────────────────────────────────
\section{Discusión}
Los resultados obtenidos muestran una concordancia satisfactoria entre las simulaciones DEM-PRM y los experimentos realizados en la pulidora horizontal instrumentada. El bajo valor de MAPE (menor al 10\%) indica que el modelo calibrado reproduce adecuadamente la tendencia de la tasa de rotura bajo variaciones de los parámetros operativos principales.

El incremento de la tasa de rotura con el aumento de la velocidad del rodillo y la reducción del espacio rodillo--criba coincide con lo reportado por Cao et al.\ \cite{cao2022} para molinos verticales industriales, aunque en este caso se valida por primera vez en una configuración horizontal de laboratorio. Además, la inclusión del coeficiente de acumulación de daño $\alpha$ permite capturar el efecto de cargas repetidas de baja energía, mecanismo dominante en el blanqueado abrasivo, en línea con lo propuesto por Tavares y King\ \cite{tavares1998} y aplicado recientemente a otros cereales por Chiaravalle et al.\ \cite{chiaravalle2025}.

Una diferencia relevante respecto a la literatura previa es la validación experimental bajo control independiente de los parámetros operativos, lo que permite identificar relaciones cuantitativas directas entre cada variable y la rotura de grano. Esto representa un avance metodológico respecto a estudios previos que consideraban solo configuraciones industriales o no incluían validación experimental detallada.

En conjunto, estos resultados preliminares demuestran la aplicabilidad del modelo DEM-PRM calibrado para el análisis y optimización de procesos de blanqueado horizontal de arroz a pequeña escala, y abren la puerta a estudios futuros de optimización paramétrica y escalamiento industrial.
